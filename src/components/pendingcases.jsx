    ...
    return(
        <div className={`${bgColor} flex h-[41px] text-[10px]  lg:text-[14px] w-full  text-[#000000] ${spacing} items-center`}>
            ...
            {id &&  <p className={`w-auto lg:min-w-[60px] bg-green-300 flex  items-start `}>{id}</p>}
             ...
             <p className={` flex items-start w-auto ${id ? "min-w-[50px] xxSmall:min-w-[40px] " : "min-w-[90px] smallSize:min-w-[80px]"} md:min-w-[80px] bg-blue-200 `}>{diagnosis}</p>
             <p className={` flex items-start  w-auto min-w-[60px] bg-green-300 lg:min-w-[80px]`}>{date}</p>
            {treatment &&   <p className={`w-auto min-w-[80px] md:min-w-[100px] lg:min-w-[135px] whitespace-nowrap overflow-x-auto bg-blue-400`}>{treatment}</p>}
