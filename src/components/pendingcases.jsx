    ...
    return(
        <div className={`${bgColor} flex h-[41px] text-[10px]  lg:text-[14px] w-full  text-[#000000] ${spacing} items-center`}>
            ...
            {id &&  <p className={`w-auto lg:min-w-[60px] flex  items-start `}>{id}</p>}
             ...
             <p className={` flex items-start md:w-auto ${id ? "w-[50px] xxSmall:min-w-[40px] " : "w-[55px] lg:min-w-[100px] smallSize:min-w-[80px] break-word"} md:min-w-[80px] whitespace-wrap bg-amber-200`}>{diagnosis}</p>
             <p className={` flex items-start  w-auto min-w-[60px] bg-red-300  lg:min-w-[80px]`}>{date}</p>
