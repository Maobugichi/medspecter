    ...
    return(
        <div className={`${bgColor} flex h-[41px] text-[10px]  lg:text-[14px] w-full  text-[#000000] ${spacing} items-center`}>
            ...
            {id &&  <p className={`w-auto lg:min-w-[60px] flex  items-start `}>{id}</p>}
             <p className={` w-auto md:min-w-[120px] min-w-[30px] bg-red-200 whitespace-wrap flex items-start break-word `}>{name}</p>    
             <p className={` flex items-start w-auto ${id ? "min-w-[50px] xxSmall:min-w-[40px] " : "min-w-[80px] lg:min-w-[100px] smallSize:min-w-[80px]"} md:min-w-[80px] whitespace-wrap  `}>{diagnosis}</p>
